	...
	*/
	...
	start = async (settings) => {
		try {
			...
			//THIS.botInfo.key = settings.apiKey;
			//THIS.botInfo.botCode = settings.botCode;
			...
			this.tg.on('poll_answer', async(ctx) => {
				console.log("poll_answer");
				await THIS.recievePollAnswer(ctx);
			});
	...
	async recievePollAnswer(ctx){
		var app = this.dictAppCollection["poll"];
		if(!app){
			console.log("Poll app is not connected");
		}

		app.recievePollAnsewr(ctx);
	}
	...
	setBotInfo(info){
		...
		this.botInfo.botCode = info.botCode;
		this.botInfo.apiKey = info.apiKey;
	}
...
Call a command of an app:
/start C-appAlias-commandName-dataToSend
*/
class AppBase {
	/**
