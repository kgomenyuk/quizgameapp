...
class Notification{
	constructor(id){
		...
		this.processing = {
			service_mode: false, // HTTP request mode
			is_processed: false,
			has_error: false,
			processing_code: ""
		};
	...
	/** set application, notification type, and other parameters
	 * 
	 */ 
...
class ScheduleManager{
	 ...
	 * @returns {boolean}
	 */
	delegateDispatchNotification = async (notif) => { 
		return true; 
	};

	constructor(){
		this.processTimer = null;// timer
		/**
		 * Enable of disable the notifications service
		 */
		this.isNotifOnline = true;
		this.serviceMode = false;
		this.appHandlerURL = null;
	}
	
	// create a new notification without writing to DB
	...
	/** Create and activate
	 * @param {String} app Name of an app that must receive the notification
	 * @param {Number} userId ID of a user who will receive the notification
	 * @param {String} nType Application-specific field
	 * @param {Number} sInterval Interval in seconds till the next notification
	 * @param {Boolean} repeat The notification will be repeated
	 * @param {Boolean} isDateTime true if exact time is used
	 * */
