...
class UIMessage {
	 ...
	 * @param { String } destinationUid User you want to send the message to
	 * @returns { UIMessage } The new message
	 */
	static messageFromString(definition, createButtons, destinationUid){
		...
		message.destinationUid = destinationUid;
	...
	/**
	 * Error text when sending the message
	 */
	errorSendingText = "";
...
class UIScreen{
	 ...
	 * @param {UIMessage} msg 
	 */
	static async updateMessageObj(ctx, msg){
 ...
 */
	 ...
	 * @param {UIMessage} msg 
	 */
	static async postMessageObj(ctx, msg){
		const userId = msg.destinationUid;
		let arrOptions = [];
		const msgTag = msg.tag;
		...
		if(UIScreen.debug == true){
			
		}else{
			if(msg.isHtml == true){
				arrOptions.push({ 
					name: "parse_mode", 
					f: (x)=>{x.parse_mode = "HTML";} 
				});
			}
			...
			// bug fix
			try{
				var m = await ctx.telegram.sendMessage(userId, text, options);
				msgId = m.message_id;
			}catch(ex){
				console.log("error when sending points. " + ex.message);
				msg.errorSendingText = ex.message;
			}
		}
		msg.id = msgId;
		msg.isPosted = true;
