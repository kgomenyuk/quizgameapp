...
class SessionObject {
	 ...
	 * Add a notification
	 * @param { String } msgDef
	 * @returns { UIMessage } Message. Will not be registered in UIScreen.
	 */
	uiNotification = (msgDef, createButtons = false)=>{
		var msg = UIScreen.textToMessage(msgDef, createButtons);
		return msg;
	};
...
class SessionManager {
	...
	fetch2 = async (uid) => {
		let iuid = this.getIUId(uid);
		var s =  this.dictSessions[iuid];
	if(s == null){
	s = await this.newSession({id:uid});
	return this.dictSessions[iuid];
	}
	return s;
	  }
