...
class ActionTrigger {
	...
	recheck(objSession, objEvent, objData) {
			return this._recheck(objSession, this._mapper(objEvent), objData);
		}
		// функция для переопределения
	_recheck(objSession, objEvent, objData) {
			return true;
		}
 ...
 * */
 class TGMessageEventTrigger extends TGEventTrigger {
	...
	_recheck(s, e) {
		if (this.filter != null) {
			const state = s.getAppState().data;
...
/**
 * триггер - ответ на опрос
 * */
class TGPollEventTrigger extends TGEventTrigger {
	constructor(id, fxFilter) {
			super(id);
			this.action = "poll_answer";
			this.filter = fxFilter;
		}
		// перепроверка условий срабатывания триггера
	_recheck(s, e) {
